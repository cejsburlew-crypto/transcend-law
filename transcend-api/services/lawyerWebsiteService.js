...
class LawyerWebsiteService {
     ...
     */
    async generateSubdomain(companyName, city, existingWebsites) {
        ...
        const duplicatesInCity = existingWebsites.filter((w) => w.subdomain.startsWith(baseSlug) &&
    ...
    async createWebsite(data) {
        ...
        const existingResult = await query('SELECT * FROM lawyer_websites ORDER BY created_at DESC LIMIT 100');
        const existingWebsites = existingResult.rows.map((row) => ({
            ...row,
            id: row.id,
        ...
        // Generate subdomain
        const subdomain = await this.generateSubdomain(data.companyName, data.city, existingWebsites);
        // Calculate included services (all except excluded)
        const includedServices = data.allAvailableServices.filter((service) => !data.excludedServices.includes(service));
        // Save to database
        const insertResult = await query(`INSERT INTO lawyer_websites (
    ...
    async updateWebsite(websiteId, updates) {
        ...
        if (updateFields.length === 1)
            return {}; // No updates
        const updateResult = await query(`UPDATE lawyer_websites SET ${updateFields.join(', ')} WHERE id = $${paramCount} RETURNING *`, updateValues);
    ...
    async getWebsiteBySubdomain(subdomain) {
        const result = await query('SELECT * FROM lawyer_websites WHERE subdomain = $1', [subdomain]);
        if (result.rows.length === 0)
            return null;
        const row = result.rows[0];
        // Fetch testimonials
        const testimonialsResult = await query('SELECT * FROM lawyer_website_testimonials WHERE website_id = $1 ORDER BY created_at DESC', [row.id]);
        const testimonials = testimonialsResult.rows.map((t) => ({
     ...
     */
    async addTestimonial(websiteId, testimonial) {
        const id = uuidv4();
        await query(`INSERT INTO lawyer_website_testimonials (
        id, website_id, client_name, rating, review, created_at
      ) VALUES ($1, $2, $3, $4, $5, $6)`, [id, websiteId, testimonial.clientName, testimonial.rating, testimonial.review, new Date()]);
        // Update audit log
        await query(`INSERT INTO lawyer_website_audit_log (
        id, website_id, lawyer_id, action, created_at
      ) SELECT $1, $2, lawyer_id, 'testimonial_added', $3 FROM lawyer_websites WHERE id = $2`, [uuidv4(), websiteId, new Date()]);
    }
    /**
     * Track page view analytics
     */
    async trackPageView(websiteId, data) {
        try {
            const today = new Date().toISOString().split('T')[0];
            // Upsert analytics record (insert if not exists, increment if exists)
            await query(`INSERT INTO lawyer_website_analytics (
          id, website_id, visit_date, page_views, unique_visitors, referral_source
        ) VALUES ($1, $2, $3, 1, 1, $4)
        ON CONFLICT (website_id, visit_date) DO UPDATE SET
          page_views = lawyer_website_analytics.page_views + 1`, [uuidv4(), websiteId, today, data.referralSource]);
     ...
     * Track service click
     */
    async trackServiceClick(websiteId, serviceId) {
        try {
            const today = new Date().toISOString().split('T')[0];
            // Upsert service click tracking
            await query(`INSERT INTO lawyer_website_service_clicks (
          id, website_id, service_name, click_date, click_count
        ) VALUES ($1, $2, $3, $4, 1)
        ON CONFLICT (website_id, service_name, click_date) DO UPDATE SET
          click_count = lawyer_website_service_clicks.click_count + 1`, [uuidv4(), websiteId, serviceId, today]);
        }
        catch (error) {
     ...
     */
    calculateSubscriptionCost(startDate, endDate, monthlyRate = 25) {
    ...
    async createCloverSubscription(data) {
        try {
            ...
            const response = await axios.post(`${CLOVER_API_BASE}/v3/merchants/${CLOVER_MERCHANT_ID}/subscription_plans`, {
     ...
     * Renew subscription
     */
    async renewSubscription(websiteId, months = 1) {
        try {
            ...
            if (website.stripe_subscription_id) {
                await axios.post(`${CLOVER_API_BASE}/v3/merchants/${CLOVER_MERCHANT_ID}/charges`, {
            ...
            const updateResult = await query(`UPDATE lawyer_websites SET
          subscription_end_date = $1,
          subscription_status = 'active',
          updated_at = $2
        WHERE id = $3
        RETURNING *`, [newEndDate, new Date(), websiteId]);
            const row = updateResult.rows[0];
            // Log billing
            await query(`INSERT INTO lawyer_website_billing (
          id, website_id, lawyer_id, amount, billing_period_start,
          billing_period_end, status, payment_method, created_at
        ) VALUES ($1, $2, $3, $4, $5, $6, 'paid', 'clover', $7)`, [uuidv4(), websiteId, website.lawyer_id, 25.00 * months, new Date(), newEndDate, new Date()]);
            // Update audit log
            await query(`INSERT INTO lawyer_website_audit_log (
          id, website_id, lawyer_id, action, new_value, created_at
        ) VALUES ($1, $2, $3, 'subscription_renewed', $4, $5)`, [uuidv4(), websiteId, website.lawyer_id, `Renewed for ${months} month(s)`, new Date()]);
            const renewedWebsite = {
                id: row.id,
                lawyerId: row.lawyer_id,
                companyName: row.company_name,
                lawyerName: row.lawyer_name,
     ...
     * Cancel subscription
     */
    async cancelSubscription(websiteId, cloverSubscriptionId) {
        try {
            // Get website from database
            const result = await query('SELECT * FROM lawyer_websites WHERE id = $1', [websiteId]);
            if (result.rows.length === 0)
            ...
            if (cloverSubscriptionId) {
                try {
                    await axios.delete(`${CLOVER_API_BASE}/v3/merchants/${CLOVER_MERCHANT_ID}/subscription_plans/${cloverSubscriptionId}`, {
                        headers: {
                            Authorization: `Bearer ${CLOVER_API_KEY}`,
                        },
                    });
            ...
            await query(`UPDATE lawyer_websites SET
          subscription_status = 'cancelled',
          updated_at = $1
        WHERE id = $2`, [new Date(), websiteId]);
            // Update audit log
            await query(`INSERT INTO lawyer_website_audit_log (
          id, website_id, lawyer_id, action, created_at
        ) VALUES ($1, $2, $3, 'cancelled', $4)`, [uuidv4(), websiteId, website.lawyer_id, new Date()]);
        }
        catch (error) {
...
//# sourceMappingURL=lawyerWebsiteService.js.map