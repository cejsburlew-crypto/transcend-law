...
exports.initializeAuditTables = initializeAuditTables;
...
exports.setRetentionPolicy = setRetentionPolicy;
exports.applyDefaultRetentionPolicies = applyDefaultRetentionPolicies;
exports.archiveOldLogs = archiveOldLogs;
exports.getComplianceReport = getComplianceReport;
exports.verifyLogIntegrity = verifyLogIntegrity;
exports.getEntityAuditTrail = getEntityAuditTrail;
exports.getAuditLogHealthCheck = getAuditLogHealthCheck;
 ...
 */
async function logActionDetailed(userId, action, entityType, entityId, options) {
    try {
        ...
        await (0, connection_1.query)(`INSERT INTO audit_logs (
 ...
 */
async function logDataAccess(userId, entityType, entityId, ipAddress, classification, sensitiveFieldsAccessed) {
 ...
 */
async function logAuthEvent(userId, eventType, ipAddress, userAgent, success = true) {
 ...
 */
async function logPermissionChange(userId, targetUserId, permissionType, changeType, ipAddress) {
...
async function searchAuditLogs(filters) {
    try {
        ...
        const result = await (0, connection_1.query)(`SELECT * FROM audit_logs ${whereClause} ORDER BY timestamp DESC LIMIT $${paramIndex + 1} OFFSET $${paramIndex + 2}`, [...params, limit, offset]);
 ...
 */
async function detectAnomalies(startDate, endDate) {
    try {
        const anomalies = [];
        // Check for multiple failed logins
        const failedLogins = await (0, connection_1.query)(`
    ...
    `, [startDate, endDate]);
        ...
        const unusualAccess = await (0, connection_1.query)(`
    ...
    `, [startDate, endDate]);
        ...
        const unusualLocations = await (0, connection_1.query)(`
 ...
 */
async function generateAuditReport(reportType, startDate, endDate, generatedBy, signReport = false) {
    try {
        ...
        // Save to database
        await (0, connection_1.query)(`INSERT INTO audit_reports (
 ...
 */
async function exportAuditLogs(filters, format, outputPath) {
    try {
 ...
 */
async function setRetentionPolicy(dataClassification, retentionDays, deletePolicy, createdBy) {
    try {
        const nextReviewDate = new Date();
        nextReviewDate.setFullYear(nextReviewDate.getFullYear() + 1);
        const result = await (0, connection_1.query)(`INSERT INTO audit_retention_policies (
      ...
      RETURNING *`, [dataClassification, retentionDays, deletePolicy, nextReviewDate, createdBy]);
 ...
 */
async function applyDefaultRetentionPolicies(adminId) {
    try {
...
async function archiveOldLogs() {
    try {
        ...
        const policies = await (0, connection_1.query)(`SELECT * FROM audit_retention_policies WHERE is_active = TRUE`);
        ...
        `, [policy.data_classification, retentionDate]);
                ...
                const deleteResult = await (0, connection_1.query)(`
        ...
        `, [policy.data_classification, retentionDate]);
          ...
          WHERE data_classification = $1 AND timestamp < $2
        `, [policy.data_classification, retentionDate]);
                deletedCount += (await (0, connection_1.query)(`
...
async function verifyLogIntegrity(logId) {
    try {
        const log = await (0, connection_1.query)(`SELECT * FROM audit_logs WHERE id = $1`, [logId]);
        if (!log.rows.length) {
            return false;
        }
 ...
 */
async function getAuditLogHealthCheck() {
    try {
        const result = await (0, connection_1.query)(`
      SELECT
        COUNT(*) as total_logs,
...
async function logAction(...args) {
    if (args.length >= 5) {
        ...
        const action = VALID.includes(rawAction)
            ? rawAction
            : 'admin';
        ...
        const options = { ipAddress: ipAddress || 'internal' };
        const extra = {};
        ...
        return logActionDetailed(args[0] || 'system', action, args[2], args[3], options);
    }
    ...
    if (args.length === 4) {
        ...
        return logActionDetailed(userId, 'admin', entityType, isOptions ? String(action) : String(last), { ipAddress: 'internal', metadata: { event: action, ...(isOptions ? last : {}) } });
    }
    ...
    const action = args[0];
    const second = args[1];
    const userId = typeof second === 'string' ? second : second?.userId;
...
async function writeAuditEntry(entryOrEvent, metadata) {
    const entry = typeof entryOrEvent === 'string'
        ? {
    ...
    const normalisedAction = VALID_ACTIONS.includes(entry.action)
        ? entry.action
        : 'admin';
    try {
...
exports.auditLogger = { log: writeAuditEntry };
async function auditLog(first, event, metadata) {
    ...
    const { userId, action, resourceId, resourceType, entityType, entityId, ipAddress, userAgent, metadata: meta, ...rest } = first;
    return writeAuditEntry({
