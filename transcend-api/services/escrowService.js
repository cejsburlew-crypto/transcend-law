...
exports.initializeEscrowAccount = initializeEscrowAccount;
exports.getEscrowAccountBalance = getEscrowAccountBalance;
exports.createEscrowHold = createEscrowHold;
exports.releaseEscrowFunds = releaseEscrowFunds;
exports.approveEscrowRelease = approveEscrowRelease;
exports.processAutomaticEscrowReleases = processAutomaticEscrowReleases;
exports.refundEscrowHold = refundEscrowHold;
exports.openDispute = openDispute;
exports.resolveDispute = resolveDispute;
exports.getDispute = getDispute;
exports.reconcileEscrowAccounts = reconcileEscrowAccounts;
exports.getReconciliationHistory = getReconciliationHistory;
exports.getEscrowHold = getEscrowHold;
exports.getEscrowHoldsByCase = getEscrowHoldsByCase;
exports.getEscrowHoldsByUser = getEscrowHoldsByUser;
...
async function initializeEscrowAccount() {
    try {
        ...
        // Save to database
        const result = await (0, connection_1.query)(`INSERT INTO escrow_accounts (stripe_account_id, balance, status)
       VALUES ($1, $2, $3)
       RETURNING *`, [account.id, 0, 'active']);
        return {
            id: result.rows[0].id,
...
async function getEscrowAccountBalance() {
    try {
        const result = await (0, connection_1.query)(`SELECT balance FROM escrow_accounts WHERE status = $1 ORDER BY created_at DESC LIMIT 1`, ['active']);
...
// ============================================
async function createEscrowHold(caseId, clientId, providerId, amount, holdPeriodDays = 30, feePercentage = 2.5, whoPaysFee = 'platform') {
    try {
        // Validate amount
        if (amount <= 0) {
        ...
        const clientResult = await (0, connection_1.query)(`SELECT stripe_customer_id FROM users WHERE id = $1`, [clientId]);
        ...
        const result = await (0, connection_1.query)(`INSERT INTO escrow_holds
       ...
       RETURNING *`, [
        ...
        // Log audit trail
        await logEscrowActivity('escrow_created', result.rows[0].id, clientId, `Escrow hold created: $${amount} for case ${caseId}`);
        return {
            id: result.rows[0].id,
            caseId,
            clientId,
...
// ============================================
async function releaseEscrowFunds(escrowHoldId, reason, releasedBy) {
    try {
        // Get escrow hold details
        const holdResult = await (0, connection_1.query)(`SELECT * FROM escrow_holds WHERE id = $1`, [escrowHoldId]);
        ...
        const providerResult = await (0, connection_1.query)(`SELECT stripe_account_id FROM providers WHERE user_id = $1`, [hold.provider_id]);
        if (providerResult.rows.length === 0) {
            throw new Error('Provider account not found');
        }
        const providerStripeAccountId = providerResult.rows[0].stripe_account_id;
        // Get escrow account
        const escrowAccountResult = await (0, connection_1.query)(`SELECT stripe_account_id FROM escrow_accounts WHERE status = $1 LIMIT 1`, ['active']);
        ...
        // Update escrow hold
        const updatedHold = await (0, connection_1.query)(`UPDATE escrow_holds
       SET status = $1, transfer_id = $2, released_at = NOW(), released_by = $3
       WHERE id = $4
       RETURNING *`, ['released', transfer.id, releasedBy, escrowHoldId]);
        // Update escrow account balance
        await (0, connection_1.query)(`UPDATE escrow_accounts
       SET balance = balance - $1
       WHERE status = $2`, [transferAmount, 'active']);
        // Log audit trail
        await logEscrowActivity('escrow_released', escrowHoldId, releasedBy, `Escrow released: $${transferAmount} to provider. Reason: ${reason}`);
        // Send notifications
...
async function approveEscrowRelease(escrowHoldId, userId) {
    try {
        // Get escrow hold
        const holdResult = await (0, connection_1.query)(`SELECT * FROM escrow_holds WHERE id = $1`, [escrowHoldId]);
        if (holdResult.rows.length === 0) {
        ...
        if (hold.client_id === userId) {
            await (0, connection_1.query)(`UPDATE escrow_holds
         SET client_approved_at = NOW()
         WHERE id = $1`, [escrowHoldId]);
        }
        ...
        // Log activity
        await logEscrowActivity('escrow_approved', escrowHoldId, userId, 'Escrow release approved');
    }
    catch (error) {
...
async function processAutomaticEscrowReleases() {
    try {
        ...
        const holds = await (0, connection_1.query)(`SELECT * FROM escrow_holds
       WHERE status = $1
       AND hold_until_date <= NOW()
       AND client_approved_at IS NOT NULL`, ['held']);
        let releasedCount = 0;
        for (const hold of holds.rows) {
            try {
                await releaseEscrowFunds(hold.id, 'Automatic release - hold period expired', 'system');
                releasedCount++;
            }
            catch (error) {
...
// REFUND LOGIC
// ============================================
async function refundEscrowHold(escrowHoldId, reason, refundedBy) {
    try {
        // Get escrow hold
        const holdResult = await (0, connection_1.query)(`SELECT * FROM escrow_holds WHERE id = $1`, [escrowHoldId]);
        if (holdResult.rows.length === 0) {
       ...
       WHERE id = $3
       RETURNING *`, ['refunded', refund.id, escrowHoldId]);
        // Update escrow account balance (add back to account)
        const refundAmount = hold.who_pays_fee === 'client' ? hold.amount + hold.escrow_fee_amount : hold.amount;
        await (0, connection_1.query)(`UPDATE escrow_accounts
       SET balance = balance + $1
       WHERE status = $2`, [refundAmount, 'active']);
        // Log activity
        await logEscrowActivity('escrow_refunded', escrowHoldId, refundedBy, `Escrow refunded: $${refundAmount}. Reason: ${reason}`);
        // Send notifications
...
// DISPUTE MANAGEMENT
// ============================================
async function openDispute(escrowHoldId, initiatedBy, reason) {
    try {
        // Get escrow hold
        const holdResult = await (0, connection_1.query)(`SELECT * FROM escrow_holds WHERE id = $1`, [escrowHoldId]);
        if (holdResult.rows.length === 0) {
        ...
        // Create dispute
        const result = await (0, connection_1.query)(`INSERT INTO escrow_disputes (escrow_hold_id, initiated_by, reason, status)
       VALUES ($1, $2, $3, $4)
       RETURNING *`, [escrowHoldId, initiatedBy, reason, 'open']);
        // Update escrow hold status
        await (0, connection_1.query)(`UPDATE escrow_holds SET status = $1 WHERE id = $2`, ['disputed', escrowHoldId]);
        // Log activity
        await logEscrowActivity('dispute_opened', escrowHoldId, initiatedBy, `Dispute opened: ${reason}`);
...
async function resolveDispute(disputeId, resolution, resolution_action, resolvedBy) {
    try {
        // Get dispute
        const disputeResult = await (0, connection_1.query)(`SELECT * FROM escrow_disputes WHERE id = $1`, [disputeId]);
        ...
        // Get escrow hold
        const holdResult = await (0, connection_1.query)(`SELECT * FROM escrow_holds WHERE id = $1`, [dispute.escrow_hold_id]);
        const hold = holdResult.rows[0];
        // Execute resolution action
        if (resolution_action === 'release') {
            await releaseEscrowFunds(dispute.escrow_hold_id, `Dispute resolved: ${resolution}`, resolvedBy);
        }
        else if (resolution_action === 'refund') {
            await refundEscrowHold(dispute.escrow_hold_id, `Dispute resolved: ${resolution}`, resolvedBy);
        }
        // Update dispute
        const updatedDispute = await (0, connection_1.query)(`UPDATE escrow_disputes
       SET status = $1, resolution = $2, resolved_at = NOW(), resolved_by = $3
       WHERE id = $4
       RETURNING *`, ['resolved', resolution, resolvedBy, disputeId]);
        // Log activity
        await logEscrowActivity('dispute_resolved', dispute.escrow_hold_id, resolvedBy, `Dispute resolved: ${resolution}`);
        return {
...
async function getDispute(disputeId) {
    try {
        const result = await (0, connection_1.query)(`SELECT * FROM escrow_disputes WHERE id = $1`, [disputeId]);
        if (result.rows.length === 0) {
            return null;
        }
...
async function reconcileEscrowAccounts() {
    try {
       ...
       WHERE DATE(created_at) = CURRENT_DATE`);
        ...
        const result = await (0, connection_1.query)(`INSERT INTO escrow_reconciliations
       ...
       RETURNING *`, [
        ...
        const accountResult = await (0, connection_1.query)(`SELECT balance FROM escrow_accounts WHERE status = $1 LIMIT 1`, ['active']);
        if (accountResult.rows.length > 0) {
            const expectedBalance = parseFloat(stats_row.total_held);
            const actualBalance = accountResult.rows[0].balance;
            if (Math.abs(expectedBalance - actualBalance) > 0.01) {
                console.warn(`⚠️  Escrow account reconciliation mismatch: expected $${expectedBalance}, got $${actualBalance}`);
            }
        }
        return {
            id: result.rows[0].id,
...
async function getReconciliationHistory(days = 30) {
    try {
        const result = await (0, connection_1.query)(`SELECT * FROM escrow_reconciliations
...
// ESCROW HOLD QUERIES
// ============================================
async function getEscrowHold(escrowHoldId) {
    try {
        const result = await (0, connection_1.query)(`SELECT * FROM escrow_holds WHERE id = $1`, [escrowHoldId]);
        if (result.rows.length === 0) {
            return null;
        }
...
async function getEscrowHoldsByCase(caseId) {
    try {
        const result = await (0, connection_1.query)(`SELECT * FROM escrow_holds WHERE case_id = $1 ORDER BY created_at DESC`, [caseId]);
...
async function getEscrowHoldsByUser(userId, userType) {
    try {
        const column = userType === 'client' ? 'client_id' : 'provider_id';
        const result = await (0, connection_1.query)(`SELECT * FROM escrow_holds WHERE ${column} = $1 ORDER BY created_at DESC`, [userId]);
        return result.rows.map((hold) => ({
            id: hold.id,
            caseId: hold.case_id,
            clientId: hold.client_id,
...
// HELPER FUNCTIONS
// ============================================
async function logEscrowActivity(action, escrowHoldId, userId, description) {
    try {
        await (0, connection_1.query)(`INSERT INTO escrow_audit_log (escrow_hold_id, action, user_id, description)
       VALUES ($1, $2, $3, $4)`, [escrowHoldId, action, userId, description]);
    }
    catch (error) {
        console.error('Failed to log escrow activity:', error);
    }
}
async function sendEscrowReleaseNotifications(hold, amount) {
    try {
        // Get user emails
        const clientResult = await (0, connection_1.query)(`SELECT email FROM users WHERE id = $1`, [
            hold.client_id,
        ]);
        const providerResult = await (0, connection_1.query)(`SELECT email FROM users WHERE id = $1`, [
            hold.provider_id,
        ]);
        if (clientResult.rows.length > 0) {
            await (0, emailService_1.sendEmailNotification)(clientResult.rows[0].email, 'Escrow Funds Released', `Your escrow hold of $${amount} has been released to the service provider.`);
        }
        if (providerResult.rows.length > 0) {
            await (0, emailService_1.sendEmailNotification)(providerResult.rows[0].email, 'Escrow Payment Received', `You have received an escrow payment of $${amount}.`);
        }
    }
    catch (error) {
...
async function sendEscrowRefundNotifications(hold, amount, reason) {
    try {
        const clientResult = await (0, connection_1.query)(`SELECT email FROM users WHERE id = $1`, [
            hold.client_id,
        ]);
        if (clientResult.rows.length > 0) {
            await (0, emailService_1.sendEmailNotification)(clientResult.rows[0].email, 'Escrow Refund Processed', `Your escrow hold of $${amount} has been refunded. Reason: ${reason}`);
        }
    }
    catch (error) {
...
async function sendDisputeNotifications(escrowHoldId, hold, reason) {
    try {
        // Notify admin
        const adminEmail = process.env.ADMIN_EMAIL || 'admin@transcend-law.com';
        await (0, emailService_1.sendEmailNotification)(adminEmail, 'Escrow Dispute Opened', `A dispute has been opened for escrow hold ${escrowHoldId}. Reason: ${reason}`);
        // Notify both parties
        const clientResult = await (0, connection_1.query)(`SELECT email FROM users WHERE id = $1`, [
            hold.client_id,
        ]);
        ...
        if (clientResult.rows.length > 0) {
            await (0, emailService_1.sendEmailNotification)(clientResult.rows[0].email, 'Escrow Dispute Opened', `A dispute has been opened on your escrow payment. Our team will investigate and resolve this matter.`);
        }
        if (providerResult.rows.length > 0) {
            await (0, emailService_1.sendEmailNotification)(providerResult.rows[0].email, 'Escrow Dispute Opened', `A dispute has been opened on your escrow payment. Our team will investigate and resolve this matter.`);
        }
    }
    catch (error) {
