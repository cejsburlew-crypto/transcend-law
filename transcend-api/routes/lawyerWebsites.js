...
import lawyerWebsiteService from '../services/lawyerWebsiteService';
 ...
 */
router.post('/api/lawyer-websites', async (req, res) => {
    try {
        const { lawyerId, lawyerName, companyName, city, bio, email, phone, licenseNumber, yearsExperience, specializations, excludedServices, allAvailableServices, profilePicture, } = req.body;
 ...
 */
router.get('/api/lawyer-websites/:websiteId', async (req, res) => {
    try {
 ...
 */
router.put('/api/lawyer-websites/:websiteId', async (req, res) => {
    try {
        const { websiteId } = req.params;
 ...
 */
router.post('/api/lawyer-websites/:websiteId/testimonials', async (req, res) => {
    try {
        const { websiteId } = req.params;
...
router.post('/api/lawyer-websites/:websiteId/cancel', async (req, res) => {
    try {
        ...
        // const website = await db.lawyerWebsites.findById(websiteId);
        await lawyerWebsiteService.cancelSubscription(websiteId, req.body.cloverSubscriptionId || 'subscription_id' // TODO: Get from database
        );
        res.json({
            success: true,
 ...
 */
router.post('/:subdomain/contact', async (req, res) => {
    try {
        const { subdomain } = req.params;
        ...
        await query(`INSERT INTO lawyer_website_contact_submissions (
        id, website_id, client_name, client_email, client_phone,
        service_interest, message, status, submitted_at
      ) VALUES ($1, $2, $3, $4, $5, $6, $7, 'new', $8)`, [submissionId, website.id, name, email, phone, serviceInterest, message, new Date()]);
 ...
 */
router.post('/:subdomain/track/service-click', async (req, res) => {
    try {
        const { subdomain } = req.params;
...
//# sourceMappingURL=lawyerWebsites.js.map